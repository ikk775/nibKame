...
\maketitle
%\section{はじめに}
我々は\emph{nibKame}という名の\emph{関数型言語}を設計し、そのコンパイラを製作した。
nibKameとは、\emph{ML}系の関数型言語である\emph{MinCaml}という言語を拡張したものである。
...
\item[アセンブリで出力] 
\end{inlineenummerate}
また、その後GNUアセンブラ(gas)によって出力したアセンブリをアセンブルし、
GNUリンカ(ld)で事前に用意したランタイムとリンクする。

現状では、構文解析部は\emph{Scheme} (処理系は\emph{Gauche}を用いた)で、それ以外は\emph{Objective Caml} (OCaml)で書かれている。
