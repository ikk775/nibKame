...
\begin{align}
    \domain X &\triangleq \pi_0\\
    \codomain X &\triangleq \pi_1
\end{align}
と定義される．

$\{x_0,x_1,\cdots,x_n\}$ ($n\in\N$)を$\vec{x}$と書く．
...
為に用いるが，今回は使用しないため本報告書では触れない．}がある．
...
\begin{array}{rcll}
    \substExpr  &\subset& \mathset{ExprVar}\times\mathset{Expr} & \text{変数の型環境} \\
    \substType  &\subset& \mathset{TypeVar}\times\mathset{Type} & \text{型変数の型環境} \\
\end{array}
\end{equation}
...
$v\in\mathset{ExprVar}$，
$e\in\mathset{Expr}$
とする．
\begin{subequations}
\begin{gather}
    \infer[\rulename{Const}]
        {\envExpr;\envType;\envVariant\vdash c:}
        {\tau'=\mathfunc{ConstType}(c) & \tau' \succ \tau} \\
    \infer[\rulename{Var}]
        {\envExpr;\envType;\envVariant\vdash v:\tau}
